...
- **Repo:** `jpm2756/carddemo-modernization-demo` (CardDemo `CBACT01C`, plus
  `CBACT02C–04C` still pending — room to show fan-out).
...
- **Integrations connected:** GitHub (native), Jira, Slack (`#general`,
  `C0BGV742NDN`), Confluence (space `CognitionD`), and a **Postgres** the DB-parity
  harness / MCP can reach.
...
**Presenter:** in project **KAN**, open the ticket *"Modernize CBACT02C"* and
**assign it to Devin** (put "use `!modernize_parity`, repo
`jpm2756/carddemo-modernization-demo`" in the description).
...
COBOL under GnuCOBOL to a **golden baseline**, writes the modern TypeScript
service, and adds the **byte-for-byte parity test**. It opens a PR, comments the
PR link + parity result back on the Jira ticket, and posts a "PR ready" note to
Slack `#general`.
...
## Scene 3 — CI proves parity (GitHub, native)

Open the PR's **Actions** run: GnuCOBOL builds the baseline, the modern service
runs, and the differential test asserts **stdout == COBOL baseline, byte-for-byte**.

> Talking point: "Every push re-derives the mainframe truth and re-checks it —
> parity is a gate, not a claim."
...
With a **Postgres MCP** connected, do this *live* from chat: ask Devin in Slack to
"show the parity diff and the balance reconciliation," and it runs the SQL and
posts the result.
...
`CBACT01C` green, `CBACT02C` now green, the rest pending — each row linking its PR
and Jira ticket. `portfolio.json` in the repo stays the machine-readable source of
truth; the page is the stakeholder view.
...
- In `#general`: `@Devin modernize the next program in
  jpm2756/carddemo-modernization-demo` or `@Devin review <PR link>` — proving
...
| Behavioral parity | GitHub Actions | stdout == COBOL baseline, byte-for-byte |
| Data parity | Postgres | 0 mismatched rows + `SUM(curr_bal)` reconciles |
...
- **Connected today:** GitHub, Jira, Slack, Confluence.
- **Add for Scene 4 (live):** a **Postgres MCP** pointed at a reachable Postgres
  (the harness + CI already prove it without the MCP).
