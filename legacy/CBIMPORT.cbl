      ...
      ******************************************************************
       ...
       FILE-CONTROL.
      * Off-platform I/O shim: the mainframe declared EXPFILE as an
      * INDEXED (KSDS) file keyed on EXPORT-SEQUENCE-NUM, but that key is
      * defined in WORKING-STORAGE (COPY CVEXPORT), not in this FD's
      * record, so GnuCOBOL (and standard COBOL) cannot resolve it. The
      * program only ever reads the file SEQUENTIALLY (the key is never
      * used for random access), so we read it as a flat SEQUENTIAL file.
      * PROCEDURE DIVISION / business logic is unchanged.
