      ...
      * Off-platform I/O shim: the mainframe declares EXPORT-OUTPUT as a
      * KSDS (INDEXED, RECORD KEY IS EXPORT-SEQUENCE-NUM). Off-platform we
      * write it as a fixed 500-byte record-sequential file. Records are
      * emitted in ascending sequence order, so the resulting bytes are
      * identical to a sequential unload of the KSDS. Business logic and
      * the 500-byte record content are unchanged.
